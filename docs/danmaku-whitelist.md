...
这份清单说明「弹幕」页会显示哪些消息。按 `2` 打开弹幕页，在「设置」中开关「在弹幕页展示其他直播间事件」。

“始终显示”的消息不受附加事件开关影响；“开启附加事件才显示”的消息默认隐藏；“始终隐藏”的消息即使开启附加事件也不会显示。未列出的消息同样隐藏，但收到的原始数据仍会保存。
...
| 分享 | 开启附加事件才显示 | 当前直播间收到的分享通知；无法据此判断分享渠道或分享是否完成。 | [ ] 意见： |
| 特别关注 | 开启附加事件才显示 | 通知名称是“特别关注”，但具体含义和出现条件还不确定，不能认定是点击某个按钮后的结果。 | [ ] 意见： |
...
以下消息即使开启附加事件也不会出现在弹幕列表中。列出它们是为了说明显示范围，不表示这些活动或功能可在应用内操作。
