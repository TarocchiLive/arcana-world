...
- **始终显示**：不受 `f` 开关影响，例如普通弹幕、礼物、醒目留言和大航海提示。
- **开启 `f` 才显示**：默认隐藏，想看更多通知时再开启。
- **始终隐藏**：开启 `f` 也不会显示。未列出的消息同样隐藏，但收到的原始数据仍会保存。

“显示”是指收到这类消息后会出现在 TUI 弹幕列表中。

大航海就是舰长、提督、总督；SC 是付费醒目留言；PK / 乱斗是主播之间的互动比赛。“新版、镜像、开放平台”用于区分同类消息的不同发送方式，普通使用不需要区分这些名字。

每项先写中文含义，括号里的名称仅供核对，不需要记住。复选框和“意见”用于记录审阅结果，不是软件开关。
...
这几种通知按内容区分，不是观众发送的普通弹幕。下面两项使用相同规则：

- [ ] 观众互动通知，按下表决定是否显示。（`INTERACT_WORD`）意见：
- [ ] 新版观众互动通知，显示规则与上项相同。（`INTERACT_WORD_V2`）意见：
...
想看抽奖、入场特效、连麦、房间统计或 PK 进展时，可以开启 `f`。目前支持的 30 项 PK 通知都在本节，默认不显示。
...
以下消息即使开启 `f` 也不显示。这里列出它们是为了说明显示范围，不表示这些活动或功能可在应用内操作。
