...
var (
	...
	winDrawText           = winUser.NewProc("DrawTextW")
	...
	winSetTextColor       = winGDI.NewProc("SetTextColor")
	winSetBackgroundColor = winGDI.NewProc("SetBkColor")
	winSetBackgroundMode  = winGDI.NewProc("SetBkMode")
...
// 指针大小字段有意依赖 Go 的原生对齐方式：这些字段在
// amd64 和 arm64 上均与 Win32 匹配（且不会在 HWND 存储中嵌入 Go 指针）。
