...
func (c *Client) PollQR(ctx context.Context, key string) (domain.LoginPoll, error) {
	...
	if d.Code == nil {
		return domain.LoginPoll{}, errors.New("二维码状态缺失")
	}
	...
	case 0:
		...
		if account.UID == "" || account.Cookies["SESSDATA"] == "" || account.Cookies["bili_jct"] == "" {
			return result, errors.New("登录响应缺少必要 Cookie，请重新扫码")
		}
		result.Account = &account
		return result, nil
	default:
		return result, &APIError{Code: *d.Code, Operation: "二维码登录"}
	}
}
