...
func (s *roomSelection) View(width, height int) string {
	...
	var labels []string
	if s.titleMode {
		s.input.Width = max(1, width-8)
		lines = append(lines, accent.Render("直播标题"), s.input.View(), muted.Render("↑↓ 历史 · Tab 填入编辑 · Enter 提交高亮历史/输入 · Esc 取消"))
		labels = s.history
		if len(labels) == 0 {
			lines = append(lines, muted.Render("暂无历史，直接输入标题后 Enter"))
		}
	} else {
		location := "全部分类 / 最近使用"
		if s.parent != "" {
		...
		lines = append(lines, accent.Render("直播分区 · "+selectionText(location)), "搜索 > "+selectionText(string(s.query)), muted.Render("输入名称/父类/ID · Ctrl+U 清搜索 · ↑↓ PgUp/PgDn Home/End · Enter/→ 进入 · ←/Esc 返回"))
		...
		if len(labels) == 0 {
			lines = append(lines, muted.Render("无匹配分区 · Ctrl+U 清除搜索"))
		}
	}
	// Keep the editor/search and a selectable row visible in short terminals.
	if height < 4 {
